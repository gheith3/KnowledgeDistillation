...
\begin{titlepage}
    ...
    \begin{tabular}{rl}
        ...
        \textbf{Code:} & \url{https://github.com/gheith3/KnowledgeDistillation} \\
    \end{tabular}
...
\textbf{Why $\beta$=8.0?} We chose this value because \citet{zhao2022decoupled} reported it as optimal in their original experiments. Our goal was to test whether their recommended setting generalizes to high-noise regimes.
...
\textbf{Why $64\times64$?} We chose this resolution because it doubles the spatial dimensions while remaining computationally feasible on our hardware. It also aligns with the minimum input size recommended for EfficientNetV2 architectures.
...
We explain why DKD collapses under strong augmentation. Consider a Mixup image: $0.8 \times \text{Image}_A + 0.2 \times \text{Image}_B$. The Teacher's logits become a linear superposition of two classes. The structured relationship between semantically similar classes (the ``dark knowledge'') is now contaminated. DKD with high $\beta$ forces the Student to learn this noisy non-target distribution via $L_{NCKD}$. The gradient signals from $L_{NCKD}$ conflict with the gradient signals from the ground truth labels. This causes training divergence.

This finding contradicts the claim that DKD is universally superior \citep{zhao2022decoupled}. We define the boundary conditions of their method: DKD is sensitive to the signal-to-noise ratio of the input data.

\paragraph{Comparison with Literature.}
Our results align with \citet{hinton2015distilling}, who showed that soft labels provide robust supervision. However, our findings contradict \citet{zhao2022decoupled}, who reported DKD as universally superior. The key difference is the augmentation regime: Zhao et al.\ used light augmentation (random crop and flip), while we used heavy augmentation (AutoAugment, Mixup, CutMix). This suggests that DKD's superiority is conditional on low-noise training environments.
...
\paragraph{Why Cross-Resolution Works.}
Upscaling a $32\times32$ image to $64\times64$ does not add new information in the information-theoretic sense. However, it changes the receptive field dynamics of the CNN. EfficientNetV2-L is pre-trained on ImageNet ($224\times224$). It is designed to detect features at specific spatial scales. Feeding it a $32\times32$ image causes feature maps to degrade to $1\times1$ too early in the network. By upscaling to $64\times64$, we delay this spatial collapse. The Teacher can utilize its deeper layers more effectively. Intuitively, this is akin to a student with poor eyesight (low-resolution) listening to a teacher describing a detailed painting (high-resolution). The student learns ``what'' is in the image better than if they only looked at a blurry version themselves.

Crucially, this high performance was achieved while the Student model continued to process data at the low $32 \times 32$ resolution. This proves that Standard KD can effectively inject high-resolution knowledge into low-resolution, cost-effective compact models. This ability to decouple training resolution from inference resolution is a key contribution for efficient model deployment.

\paragraph{Implication for Green AI.}
The Cross-Resolution method aligns with the goals of Green AI and Edge Computing:
\begin{itemize}
    \item \textbf{Training Cost:} Increased slightly (Teacher processes $64\times64$).
    \item \textbf{Inference Cost:} Unchanged (Student remains $32\times32$).
    \item \textbf{Performance:} Improved by 1.74\%.
\end{itemize}
...
\paragraph{Comparison with Literature.}
Cross-resolution distillation is underexplored in the literature. Most KD papers assume matched resolutions between Teacher and Student \citep{gou2021knowledge}. \citet{mirzadeh2020improved} introduced Teacher Assistants to bridge capacity gaps, but they did not explore resolution gaps. Our approach aligns with the findings of \citet{touvron2019fixing}, who demonstrated that decoupling training and inference resolutions can unlock latent model capacity. Our work extends this insight to the Teacher-Student paradigm, showing that resolution mismatch can be beneficial when the Teacher operates at higher resolution.
...
\begin{enumerate}
    \item \textbf{Single Architecture:} We used only EfficientNetV2 (L and S variants). The findings may differ for other architectures such as ResNet or Vision Transformers.
    \item \textbf{Single Dataset:} We evaluated on CIFAR-100 only. Generalization to larger datasets (e.g., ImageNet) requires further validation.
    \item \textbf{Limited Resolution Range:} We tested Cross-Resolution only between $32\times32$ and $64\times64$. Larger gaps (e.g., $128\times128 \to 32\times32$) remain unexplored.
\end{enumerate}
...
\textbf{Interpretation:} Figure~\ref{fig:loss_v2_v3} shows that DKD (v3) exhibits erratic loss behavior after epoch 50. The loss curve diverges, triggering early stopping at epoch 84. In contrast, Standard KD (v2) maintains stable convergence throughout training. This confirms the Regularization-Distillation Conflict.

\vspace{-1em}
% Figure 1b: Loss Comparison v2 vs v4 (Cross-Resolution)
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../../code_v2_32/figures/fig1b_cross_resolution.pdf}
\vspace{-1em}
\caption{Cross-Resolution Analysis: v4 (64$\times$64 Teacher) achieves 77.93\% vs v2 (32$\times$32 Teacher) at 76.19\%.}
\label{fig:cross_res}
\end{figure}

\textbf{Interpretation:} Figure~\ref{fig:cross_res} demonstrates that v4 (Cross-Resolution) achieves a higher final accuracy than v2. Both curves show stable convergence, but v4 benefits from the stronger $64\times64$ Teacher. This proves that the Student's performance ceiling is determined by Teacher quality, not Student capacity.
...
\textbf{Interpretation:} Figure~\ref{fig:dkd_beta} shows the effect of reducing $\beta$ from 8.0 to 2.0. The lower $\beta$ reduces reliance on noisy non-target logits, allowing partial recovery. However, even the tuned DKD ($\beta$=2.0) underperforms Standard KD by 0.56\%.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../../code_v2_32/figures/fig5_zoomed_convergence.pdf}
\vspace{-1em}
\caption{Zoomed Convergence (Final 50 Epochs): v4 (77.93\%) surpasses v2 (76.19\%) and v1 (76.12\%).}
\label{fig:zoomed_convergence}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Expand Cross-Resolution Study:} Test larger resolution gaps (e.g., $128\times128$ Teacher $\to$ $32\times32$ Student). We aim to find the point where the resolution gap becomes too wide for the Student to bridge.
    \item \textbf{Mathematical Formalization:} Derive the closed-form expectation of the $L_{NCKD}$ gradient under Mixup. This would provide a mathematical proof of the variance increase that causes DKD collapse.
\end{enumerate}
...
\bibitem[Touvron et al.(2019)]{touvron2019fixing}
Touvron, H., Vedaldi, A., Douze, M., and J\'{e}gou, H. (2019).
\newblock Fixing the train-test resolution discrepancy.
\newblock In \emph{Advances in Neural Information Processing Systems (NeurIPS)}, pages 8252--8262.
\newblock \url{https://arxiv.org/abs/1906.06423}

\bibitem[Gou et al.(2021)]{gou2021knowledge}
Gou, J., Yu, B., Maybank, S.~J., and Tao, D. (2021).
\newblock Knowledge distillation: A survey.
\newblock \emph{International Journal of Computer Vision}, 129(6):1789--1819.
\newblock \url{https://arxiv.org/abs/2006.05525}
