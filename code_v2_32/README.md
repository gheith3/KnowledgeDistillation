...
| :--- | :--------- | :----------- | :------------------------------ | :---------------------- |
| -    | Teacher    | -            | EfficientNetV2-L baseline       | ~75-76%                 |
| v1   | Baseline   | Standard KD  | Mixup + CutMix only             | ~72%                    |
| v2   | Enhanced   | Standard KD  | + AutoAugment + Label Smoothing | ~74% ✓                  |
| v3   | DKD β=8.0  | Decoupled KD | Default DKD parameters          | ~72% (over-regularized) |
| v3.1 | DKD β=2.0  | Decoupled KD | Tuned beta parameter            | ~74%                    |
| v4   | Saturation | Standard KD  | Strong teacher + Standard KD    | ~74% (saturation)       |
...
1. **Standard KD outperforms DKD** under strong augmentation (74.20% vs 72.69%)
2. **DKD is sensitive to beta parameter** - reducing from 8.0 to 2.0 improves performance
3. **Capacity saturation exists** - stronger teachers don't always improve students
4. **AutoAugment + Label Smoothing** significantly improve results
